...
class Account(SQLModel, table=True):
    ...
    account_id: str = Field(description="Platform-specific account identifier (e.g., user@instance.social)")
    display_name: Optional[str] = Field(default=None)
    avatar_url: Optional[str] = Field(default=None)
    # Encrypted credentials
    encrypted_credentials: str = Field(description="Encrypted OAuth token or app password")
    # Platform-specific metadata
    instance_url: Optional[str] = Field(default=None, description="Mastodon instance URL")
    bluesky_handle: Optional[str] = Field(default=None, description="Bluesky handle (without @)")
    # Status
...
class ScheduledPost(SQLModel, table=True):
    ...
    content: str = Field(min_length=1, max_length=500, description="Post content (plain text)")
